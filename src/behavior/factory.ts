...
export async function createBehaviorTimeline(
  ...
  if (options.strategy === 'llm-preferred' && options.llmProvider) {
    try {
      const candidate = await options.llmProvider.generate(behaviorInput);
      const candidateWithSections = {
        ...candidate,
        modules: splitModulesAtFlexibleSectionBoundaries(candidate.modules, behaviorInput)
      };
      ...
      return createRuleFallback(behaviorInput, validation.warnings);
    } catch (error) {
